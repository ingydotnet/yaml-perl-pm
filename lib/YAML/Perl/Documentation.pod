...
PyYAML is a YAML toolset written in Python. It is considered to be the
YAML reference implementation. YAML::Perl is a pure Perl port of PyYAML.

The PyYAML documentation can be found at
L<http://pyyaml.org/wiki/PyYAMLDocumentation>.

This document is a "port" of that page. I took the original wiki text of
the page, converted it to POD and changed the content to be correct for
YAML::Perl.

This document is not intended to be the all encompassing documentation
for YAML::Perl or YAML usage in Perl. It is just a translation of the
PyYAML work.

Please see these additional readings:

    * ...
    * ...

=head1 YAML::Perl Documentation

I<<RPG-ish descriptions are stolen from L<the Angband rogue-like game|http://www.thangorodrim.net/>.
Names of the heroes are generated with
L<MudNames|http://www.ibiblio.org/pub/Linux/games/muds/mudnames-1.5.lsm>.>>

I<This documentation is very brief and incomplete. Feel free to fix or improve
it.>

=head2 Installation

Download the source package B<YAML-Perl-x.xx.tar.gz> and unpack it. Go to the
directory B<YAML-Perl-x.xx> and run:
...
Perl version, you need to download and install L<YAML::XS>.

Note that as of this time, YAML::XS only supports the Dump/Load API, not the
full YAML stack like YAML::Perl does. If you need the fancier features, like
streaming parsing, stick with YAML::Perl for now. At some point, YAML::XS will
be made to do all the same APIs as YAML::Perl.

Note that there are some subtle (but not really significant) differences
between pure Perl and YAML::XS based parsers and emitters.

=head2 Tutorial

Start with importing the C<YAML::Perl> module.
...
'''`yaml.load`''' accepts a byte string, a Unicode string, an open binary file object,
or an open text file object. A byte string or a file must be encoded with '''utf-8''',
...
'''`yaml.dump`''' accepts the second optional argument, which must be an open text
or binary file.  In this case, '''`yaml.dump`''' will write the produced YAML document
...
[http://yaml.org/spec/1.1/#id857168 Chapter 2 of the YAML specification].
...
In Python 3, `str` objects are converted to `!!str` scalars and `bytes` objects to `!!binary` scalars.
For compatibility reasons, tags `!!python/str` and `!!python/unicode` are still supported and converted
to `str` objects.
...
using the [wiki:LibYAML] library.
...
'''`Loader.get_token()`''' returns the next token in the stream and removes
it from the internal token queue. The function returns `None` at the end of the stream.
...
'''`Loader.get_event()`''' returns the next event in the stream and removes
it from the internal event queue. The function returns `None` at the end of the stream.
...
'''`Loader.get_node()`''' construct the representation graph of the next document in the stream and
...
'''`Loader.get_data()`''' constructs and returns a Python object corresponding to the next document
in the stream.
...
'''`Dumper.represent_sequence(tag, sequence, flow_style=None)`''' return a sequence node with the given `tag`
and subnodes generated from the items of the given `sequence`.

'''`Dumper.represent_mapping(tag, mapping, flow_style=None)`''' return a mapping node with the given `tag`
